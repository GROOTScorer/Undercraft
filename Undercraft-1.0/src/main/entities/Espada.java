...
public class Espada {
	...
	public Espada(int tipoEspada, Player player, List<Zombie> zombies, List<Skeleton> skeletons, List<Creeper> creepers) {
	...
	public void update() {
		...
		if (golpeDado) {
		    ...
		    for (Creeper enemigo : creepers) {
		        if (isCollidingWithEnemy3(enemigo)) {  // Verificar colisión
	...
	private boolean isCollidingWithEnemy3(Creeper creepers) {
        int swordX = (ladoEspada == 2) ? player.getX() + player.getWidth() : player.getX() - 20;
        int swordWidth = 40; // Ancho de la espada
        int swordHeight = player.getHeight(); // Altura de la espada
        ...
        // Rectángulo del enemigo
        int enemyX = creepers.getX();
        int enemyY = creepers.getY();
        int enemyWidth = creepers.getWidth();
        int enemyHeight = creepers.getHeight();
